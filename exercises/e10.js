...
export const result1 = Promise.all(promiseArr).catch((err) => {
  console.log(err);
  return err;
}); // Your code here
...
export const result2 = Promise.any(promiseArr).then((values) => {
  console.log(values);
  return values;
}); // Your code here
...
export const result3 = Promise.allSettled(promiseArr).then((values) => {
  console.log(values);
  return values;
}); // Your code here
...
export const newPromiseArr = promiseArr.map(
  (promise, index) =>
    new Promise((res) => setTimeout(res, index === 4 ? 0 : 4000, promise))
); // Your code here
